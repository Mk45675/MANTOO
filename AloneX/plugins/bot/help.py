...
async def helper_private(client: app, update: Union[types.Message, types.CallbackQuery]):
    ...
    if is_callback:
        ...
        await update.reply_video(
            video="https://files.catbox.moe/53cucv.mp4",
...
async def helper_private(client: app, update: Union[types.Message, types.CallbackQuery]):
    ...
    if is_callback:
        ...
        _ = get_string(language)
        keyboard = help_pannel(_)
        await update.reply_video(video="https://files.catbox.moe/53cucv.mp4", caption=_["help_1"].format(SUPPORT_CHAT), reply_markup=keyboard)
